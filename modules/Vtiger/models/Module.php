...
class Vtiger_Module_Model extends Vtiger_Module {
    ...
    */
	...
	//Model para capiturar os chamados de um cliente, no meu caso os chamados de uma organização.

	public function getTicket($id = null) {
		
		$db = PearDatabase::getInstance();

		$result = $db->pquery('select t.ticketid, t.title, t.ticket_no from vtiger_troubletickets t, vtiger_account a
		where a.accountid = t.parent_id
		and a.accountid = '.$id.' LIMIT 10;');
