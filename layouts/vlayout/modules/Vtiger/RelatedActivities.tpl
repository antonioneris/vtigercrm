...
{* Como nao consegui encontrar uma forma de abrir a view 

	Resolvi usar o mesmo template de atividades para chamar o template dos chamados
...
<div  class="summaryWidgetContainer">
	<div class="widget_header row-fluid">
		<span class="span8"><h4 class="textOverflowEllipsis">{vtranslate('Ultimos Chamados',$MODULE_NAME)}</h4></span>
	</div>
	<div class="widget_contents">
		{if count($TICKETS) neq '0'}
			{foreach item=RECORD key=KEY from=$TICKETS}
				<div class="activityEntries">
					{$RECORD['title']} <b>Ticket</b> N: <a href="index.php?module=HelpDesk&view=Detail&record={$RECORD['ticketid']}"> {$RECORD['ticket_no']} </a> 
				</div>	
				<hr>
			{/foreach}
		{else}
			<div class="summaryWidgetContainer">
				<p class="textAlignCenter">{vtranslate('Nenhum Chamado aberto',$MODULE_NAME)}</p>
			</div>
		{/if}
		{if $PAGING_MODEL->isNextPageExists()}
			<div class="row-fluid">
